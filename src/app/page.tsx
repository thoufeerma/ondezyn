...
export default function Home() {
  ...
  useEffect(() => {
    const interval = setInterval(() => {
      setProgress((prev) => {
        ...
        // Smaller, more frequent increments for a buttery smooth progress bar
        return prev + Math.floor(Math.random() * 3) + 1;
      });
    }, 20); // Much faster tick rate (20ms instead of 80ms)
  ...
  return (
    <>
      ...
      {loading && (
        <div
          className={`fixed inset-0 bg-[#050505] z-[10000] flex flex-col justify-center items-center transition-all duration-[1200ms] ease-[cubic-bezier(0.22,1,0.36,1)] ${
            progress === 100 ? 'opacity-0 pointer-events-none' : 'opacity-100 pointer-events-auto'
          }`}
        >
          {/* Subtle background glow for premium feel */}
          <div className={`absolute inset-0 bg-[radial-gradient(ellipse_at_center,rgba(212,175,55,0.03)_0%,transparent_70%)] transition-opacity duration-1000 ${progress === 100 ? 'opacity-0' : 'opacity-100'}`}></div>
          
          <div className={`mb-10 transition-transform duration-[1200ms] ease-out ${progress === 100 ? 'scale-110 blur-[2px]' : 'scale-100 blur-0'} animate-float-slow relative z-10`}>
            <Image src="/images/logo black.webp" alt="ONDEZYN Logo" width={450} height={200} className="object-contain" priority unoptimized />
          </div>
          
          <div className={`w-[250px] sm:w-[300px] h-[2px] bg-white/5 overflow-hidden rounded-full relative z-10 transition-opacity duration-[800ms] ease-out ${progress === 100 ? 'opacity-0' : 'opacity-100'}`}>
            <div 
              className="h-full bg-gradient-to-r from-accent-red/80 via-accent-orange to-accent-gold rounded-full transition-all duration-[40ms] ease-linear" 
